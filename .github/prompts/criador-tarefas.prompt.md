...
- Amarrar cada tarefa principal às user stories do PRD sempre que possível, distinguindo cobertura direta, parcial ou de suporte
...
- [US-XX] [Título da user story] ([cobertura direta|cobertura parcial|suporte])
...
- Evidência esperada: [critérios de aceite, artefatos, testes ou docs que provam a cobertura]
...
- Sempre que houver user stories no PRD, inclua a seção `Relacionada às User Stories` em cada task e a seção `Rastreabilidade US -> Tasks` no `tasks.md`
